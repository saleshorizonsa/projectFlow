  ...
  return (
    <>
      <Dialog open onOpenChange={(o) => {
        if (!o && isDirty) { setDiscardOpen(true); }
        else if (!o) { onClose(); }
      }}>
            ...
            {type === "milestone" && (
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1" onClick={() => { if (isDirty) { setDiscardOpen(true); } else { onClose(); } }}>Cancel</Button>
      ...
      <DiscardChangesDialog open={discardOpen} onKeep={() => setDiscardOpen(false)} onDiscard={() => { setDiscardOpen(false); onClose(); }} />
    </>
  );
}
