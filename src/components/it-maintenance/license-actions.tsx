      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          ...
          <form className="grid gap-4 md:grid-cols-2" onSubmit={save}>
            <Field label="License ID" id="licenseId"><Input id="licenseId" name="licenseId" defaultValue={license.licenseId} /></Field>
            <Field label="Name" id="name"><Input id="name" name="name" defaultValue={license.name} /></Field>
            <Field label="Vendor" id="vendor"><Input id="vendor" name="vendor" defaultValue={license.vendor} /></Field>
            <Field label="Seats / Qty" id="seats"><Input id="seats" name="seats" type="number" min="1" defaultValue={license.seats} /></Field>
            <Field label="Cost" id="cost"><Input id="cost" name="cost" type="number" min="0" step="0.01" defaultValue={license.cost} /></Field>
            <Field label="Expiry Date" id="expiryDate"><Input id="expiryDate" name="expiryDate" type="date" defaultValue={license.expiryDate.slice(0, 10)} /></Field>
            <Field label="Owner" id="owner"><Input id="owner" name="owner" defaultValue={license.owner} /></Field>
            <Picker label="Linked Asset" value={assetId} onValueChange={setAssetId} items={[{ value: "none", label: "No asset" }, ...assets.map((asset) => ({ value: asset.id, label: `${asset.assetTag} / ${asset.name}` }))]} />
            <Picker label="Employee Assignee" value={employeeId} onValueChange={setEmployeeId} items={[{ value: "none", label: "Unassigned" }, ...employees.map((employee) => ({ value: employee.id, label: `${employee.employeeId} / ${employee.name}` }))]} />
            <Field label="Notes" id="notes"><Input id="notes" name="notes" defaultValue={license.notes ?? ""} /></Field>
