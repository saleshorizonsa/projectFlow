...
export default async function TasksPage({ searchParams }: { searchParams?: Promise<CompanySearchParams> }) {
  ...
  const taskRows = tasks.map((task) => ({
    ...
    layer: { name: task.layer.name },
    subLayer: { name: task.subLayer.name },
  }));
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <CardDescription>Kanban, list, calendar, and Gantt views for layer-level execution.</CardDescription>
          </div>
